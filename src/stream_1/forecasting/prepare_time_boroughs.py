...
def prepare_monthly_boroughs(ADJUST_COVID = False):
...
def prepare_quarterly_boroughs(ADJUST_COVID = False):
    ...
    borough_df = raw.groupby(['LA_2023', 'LA_Name', 'year', 'quarter']).agg(
        ...
        IMD_mean=('IMD10', 'mean'),
        LondInOut=('LondInOut', 'first')
    ).reset_index().sort_values(['LA_2023', 'year', 'quarter']).reset_index(drop=True)
    borough_df['LA_Name'] = borough_df['LA_Name'].replace({
        'Kingston upon Thames': 'Kingston',
        'Richmond upon Thames': 'Richmond'
    })
    borough_df['LondInOut'] = borough_df['LondInOut'].map({1.0: 'Outer', 2.0: 'Inner'})
    borough_df = borough_df[borough_df['LA_2023'] != 59.0]
    borough_df.sort_values(['LA_2023', 'year', 'quarter'], inplace=True)
    print("COMPLETE")
    return borough_df
...
def prepare_yearly_boroughs(ADJUST_COVId = False):
    ...
    borough_df = raw.groupby(['LA_2023', 'LA_Name', 'year']).agg(
        ...
        IMD_mean=('IMD10', 'mean'),
        LondInOut=('LondInOut', 'first')
    ).reset_index().sort_values(['LA_2023', 'year']).reset_index(drop=True)
    borough_df['LA_Name'] = borough_df['LA_Name'].replace({
        'Kingston upon Thames': 'Kingston',
        'Richmond upon Thames': 'Richmond'
    })
