...
"""
File contains the HDBSCAN class used to generate the HDBSCAN cluster labels
Such labels are used for both sports profiling and UMAP-HDBSCAN clustering

Grid search approach, optimising for DBCV, was employed to find optimal hdb hyperparams
Class contains, widened search for larger scale dataset clustering and narrowed search for smaller scale sports clustering
...
class GenerateHDBSCAN:
    ...
    def __init__(self):
        ...
        self.save_path = ROOT / 'results' / 'stream_3' / 'hdb_clusters'
        self.output_df = None
        self.dbcv = None
    ...
    def fit_narrowed(self, emb : np.array):
        optimal_hp = self.tune_narrowed_hyperparams(emb)
